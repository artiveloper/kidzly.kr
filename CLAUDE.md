...
| 2026-09-06 | **허브 중복 회피를 리서치 단계의 필수 산출물로 신설** — content-researcher가 같은 클러스터 허브 `.mdx`를 전수로 읽고 브리프에 "중복 회피 목록"(항목별로 재서술/링크 위임 판정)을 남긴다. 계획서 각도의 절반 이상이 이미 다뤄졌으면 각도 재정의를 보고하고 대안 2개 이상을 제시한다. seo-writer는 링크 위임 항목을 본문에서 다시 쓰지 않고, fact-checker에 Step 5(허브 중복 점검)를 추가해 재서술을 🔄로 잡는다 | agents/content-researcher.md, agents/seo-writer.md, agents/fact-checker.md, skills/content-creator/SKILL.md, docs/content/발행-완료.md | B1·A3·D2가 세 번 연속 계획된 각도와 허브 글이 겹쳐 집필 직전에 각도를 틀었고, D2는 그대로 갔으면 70%가 중복될 뻔했다. 발행 계획의 각도는 허브가 쓰이기 전에 잡은 것이라 시간이 지날수록 어긋난다. 같은 사이트 안의 두 글이 같은 주제를 다루면 서로의 검색 순위를 갉아먹으므로 외부 경쟁 글 조사보다 이쪽이 실제 위험이 크다 |
| 2026-09-06 | `readingTime`을 velite에서 자동 산출(공백 제외 글자 수 ÷ 500, 올림, 최소 1분)하고 frontmatter 필드를 스키마·25편 전체에서 제거. 썸네일 "쓰면 안 되는 이모지" 목록(글리프 내 영문 텍스트·축소 시 붕괴·칩 색 충돌) 7종과 선택 기준 3가지, 112px 축소 검수 절차를 명문화 | apps/web/velite.config.ts, apps/web/content/blog/*.mdx, skills/content-creator/references/image-html-template.md, agents/image-director.md, agents/seo-writer.md, agents/content-integrator.md, docs/content/README.md, docs/content/발행-완료.md | 읽기 시간을 사람·에이전트가 매번 추정해 분량과 무관한 값이 섞여 있었다(5,007자 20분, 3,413자 15분, 3,376자 9분). 썸네일은 이모지 선택 실패로 매번 2\~4회 재작업이 돌았고 원인이 세 가지로 반복돼 목록화가 가능했다 |
| 2026-09-09 | **문체를 합쇼체(`~합니다`) → 해요체(`~해요`)로 전환**하고 강조 방식을 이원화(개념어는 `'작은따옴표'`, 부모가 실제로 말할 질문·행동 문장만 **굵게**, 항목당 굵은 문장 1개 원칙). 문장 끝을 `~고요.`로 흐리지 않는 규칙, `돼요`/`예요`·`이에요` 구분 추가. 톤 기준 글을 어린이집-질병별-등원기준 → **어린이집-오리엔테이션-질문**으로 변경. 파일럿으로 오리엔테이션 1편만 변환(나머지 24편은 합쇼체 유지, 확대는 후속 결정) | agents/seo-writer.md, agents/content-integrator.md, apps/web/content/blog/어린이집-오리엔테이션-질문.mdx, docs/content/오리엔테이션-토스어투-샘플.md | 토스피드(tossfeed) 5개 글의 어투를 참고해 부모에게 직접 말 거는 친근한 안내 톤으로 바꾸기로 함. 기존 합쇼체는 YMYL 권위 신호였으나, 실제 이탈 방어에는 친근·공감 톤이 낫다고 판단. 강조를 굵게만 쓰던 방식이 "개념"과 "그 자리에서 말할 문장"을 구분하지 못해 스캔성이 떨어지던 문제도 함께 정리 |
| 2026-09-11 | **읽기 시간 8~12분 밴드를 편집 기준으로 명문화**하고 12분 초과 4편(낮잠시간 16분·질병별 등원기준·하루일과 연령별·준비물 연령별 각 13분)을 곁가지·서술 깊이 압축으로 전부 12분에 안착. 완결성·자체 데이터·출처·정부 인용문·내부 링크·이미지는 보존. 밴드는 랭킹·분량 목표가 아니라 모바일 이탈 방어(UX)이며 12분 초과 글만 대상, 8분 미만은 패딩하지 않는다는 원칙을 README §4·seo-writer 완결성 섹션에 반영 | apps/web/content/blog/{어린이집-낮잠시간,어린이집-질병별-등원기준,어린이집-하루일과-연령별,어린이집-준비물-연령별}.mdx, docs/content/README.md, docs/content/읽기시간-고도화-TODO.md, agents/seo-writer.md | 해요체 전환으로 글자 수가 바뀌며 12분 초과가 5편으로 늘었고, 상위 글의 13~18분 읽기 시간이 모바일 중간 이탈을 유발한다고 판단. 밴드 기준이 그동안 TODO 문서에만 있어 신규 글에 적용되지 않던 문제도 함께 정리. 길이는 구글 랭킹과 무관하므로 완결성을 깎지 않고 곁가지만 걷는 선으로 한정 |
| 2026-09-11 | **해요체를 전 발행글로 확대 확정** — 2026-09-09 파일럿의 "확대는 후속 결정"을 실행했다. 나머지 24편의 본문과 frontmatter `description`을 합쇼체 → 해요체로 변환하고, 합쇼체로 끝나던 H2/H3 제목도 해요체로 통일. 정부 매뉴얼·지침 원문 인용문과 참고자료 문서 제목(고유명사)은 원문 그대로 유지. **강조는 지원금·제도 안내 글에서는 기존 숫자·키워드 굵게를 유지**하기로 결정 — 오리엔테이션의 "말할 질문·행동 문장만 굵게" 재설계는 질문 12개로 구성된 그 글 구조에 한정하고, 제도 안내 글에는 문체 변환만 적용한다. 오리엔테이션 발행본에 남아 있던 규칙 위반 굵게 1개 제거. 전 25편 해요체 통일 완료, `velite build` 에러 0건 | apps/web/content/blog/*.mdx (24편 변환 + 어린이집-오리엔테이션-질문 동기화) | 2026-09-09 파일럿 톤을 사용자가 최종 채택해 사이트 전체 톤 일관성을 확보. seo-writer·content-integrator는 이미 해요체 규칙을 반영해 두어 에이전트 변경은 없고, 기존 글 소급 적용만 남아 있던 상태였다. 강조 규칙을 전 글에 소급하면 제도 안내 글의 숫자·금액 강조를 모두 재설계해야 해 스캔성이 오히려 떨어진다고 판단, 문체와 강조의 적용 범위를 분리했다 |
| 2026-09-12 | **닫는 `**` 앞 구두점으로 볼드가 렌더링되지 않던 12곳 수정**(오리엔테이션 11곳 `**'질문'**조사`, 가정양육수당 1곳 `…끊기나요?**라는`) — 구두점을 굵게 밖으로 빼는 방식으로 통일. 규칙을 seo-writer 강조 예시·MDX 포맷 규칙에 반영하고, 볼드 미적용은 빌드 에러를 내지 않으므로 발행 게이트에 `grep -c '\*\*' apps/web/.velite/blog.json` → `0` 확인 단계를 추가 | apps/web/content/blog/{어린이집-오리엔테이션-질문,가정양육수당-알아보기}.mdx, agents/seo-writer.md, agents/content-integrator.md, skills/content-creator/SKILL.md | CommonMark right-flanking 규칙상 닫는 `**` 앞이 구두점이고 뒤가 한글 조사면 볼드가 걸리지 않아 `**`가 화면에 그대로 노출됐다. 기존 규칙은 `)` 괄호 사례만 다루고 있었고, seo-writer의 강조 예시 자체가 깨진 형태(`**'…'**`)라 에이전트가 그대로 재생산하고 있었다. 한국어는 조사가 거의 항상 붙어 재발 확률이 높은데 velite 빌드는 에러 없이 통과해 자가 점검으로만 막을 수 없었다 |
...
| 2026-08-19 | 구글 공식 문서 크로스체크 반영 — §1.4 구조화 데이터 오류 3건 수정(ChildCare 타입 존재, `additionalType` 미지원, `image` 요건), §1.5 AI 항목을 공식 `ai-optimization-guide` 기준으로 재작성, **§1.7 "구글 공식 규정" 신설**(원문 대조 확인 항목만 수록, 감사에서 그대로 인용 가능), 출처를 공식/3자로 등급 분리 | skills/seo-manager/references/naver-daum-google-seo-guide.md, CLAUDE.md | 정책 문서 출처 20개 중 구글 공식 문서가 1개뿐이라 사이트맵 `lastmod`·숨겨진 텍스트 예외·`ItemList` 필수 속성 같은 규정이 통째로 빠져 있었고, 그 결과 직전 감사가 P1 5건을 놓쳤다. 산출물은 [docs/seo/구글-공식문서-크로스체크.md](docs/seo/구글-공식문서-크로스체크.md) |
