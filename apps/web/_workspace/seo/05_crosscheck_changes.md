...
## 3차 반영 — P2-1 `priority`/`changefreq` 제거 (2026-08-19)

**조사 결과 — 이 값을 사용한다고 문서화한 엔진이 없다.**

- sitemaps.org 규격 원문: `priority`는 "검색 엔진의 결과 페이지에서 URL의 순위에 별 영향을 미치지 않습니다", `changefreq`는 "값은 힌트이지 명령이 아닙니다". 둘 다 선택 태그다.
- 구글: "`<priority>` 및 `<changefreq>` 값을 무시합니다."
- 네이버: 서치어드바이저 공식 가이드는 접근 불가(차단). 국내 사이트맵 가이드들을 조사했으나 **네이버가 이 값을 사용한다는 기술은 찾지 못했다.** 오히려 "`loc`과 `lastmod`가 핵심"이라고만 쓴다.
- 다음: 사이트맵 기술 규격 문서 자체가 없다. 도움말은 UI 등록 절차만 안내한다.

**제거 근거 두 가지**

1. 크기 — 실측 3.81MB 중 **1.27MB(33%)**가 두 태그였다. 50MB 한도와는 무관하지만 크롤러가 매번 받아 파싱하는 양이다.
2. 유지보수 비용 — 직전 감사(8/16)가 P2 슬롯 하나를 써서 `changeFrequency`를 `weekly` → `daily`로 "정확하게" 고쳤다. 아무도 읽지 않는 값에 감사 슬롯이 소모된 셈이다.

**반영:** `app/sitemap.ts`에서 두 필드 전부 제거, 재발 방지를 위해 파일 상단 주석과 정책 문서 §1.7에 근거를 남겼다.

**검증:** 재빌드 후 `priority` 0건 / `changefreq` 0건, URL 24,634개·`lastmod` 24,630개 유지, 파일 크기 **3.81MB → 2.49MB**.

---

## 남은 항목

- **P2-7 제목 반복** — **철회했다.** 실제 렌더된 제목을 확인하니 홈은 "어린이집" 1회(3회라던 초기 리포트가 부정확했다), 2회인 건 `/map` 하나뿐이고 그마저 자연스러운 한국어다. 구글 문서는 사이트명을 제목 끝에 구분자와 함께 넣는 것을 오히려 권장하며, 반복 시 구글이 제목 링크에서 알아서 생략한다. 위반이 아니라 표시 공간 문제라 조치하지 않는다.
