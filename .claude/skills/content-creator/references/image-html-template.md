...
**색상·이모지 선택 규칙**
- 아티클의 핵심 주제 키워드 하나를 골라 대표 이모지를 정한다 (예: 집=🏠, 이용권/선물=🎁, 아이=👶, 급여/돈=💰, 일정=📅, 병원=🏥, 책=📚, 체크=✅)
- 같은 카테고리(예: "영유아 지원금") 안에서도 아티클마다 색을 다르게 배정한다 — 목표는 리스트에서 나란히 봤을 때 전부 다른 색으로 보이는 것
- 직전 발행 3편의 썸네일(`apps/web/public/blog/{슬러그}/thumbnail.png`)을 Read로 열어 색이 겹치지 않는지 확인한다
- 이 레이어에는 절대 텍스트를 넣지 않는다 (96px에서 읽히지 않음)
- 이모지 폰트 크기는 항상 칩(820px) 대비 약 51%(420px)로 고정 — 더 작으면 리스트에서 존재감이 사라짐

**쓰면 안 되는 이모지 (실제로 걸렸던 것들)**

애플 컬러 이모지 글리프 중 일부는 그림 안에 영문 텍스트가 그려져 있거나, 형태가 96\~112px에서 무너진다.
아래는 실패가 확인된 목록이다. **재시도로 시간을 쓰지 말고 처음부터 후보에서 뺀다.**

| 이모지 | 문제 | 확인일 |
|--------|------|--------|
| 📋 클립보드 | 종이 위에 영문 문단("Here's to the crazy ones…")이 그려져 있다. 한국어 블로그 썸네일에 영문이 노출된다 | 2026-09-05 |
| 📄 페이지 | 같은 영문 레터 텍스트가 그려져 있다 | 2026-09-06 |
| 🧾 영수증 | "RECEIPT" 등 영문이 인쇄돼 있다 | 2026-09-06 |
| 📝 메모 | 디테일이 잘아서 축소하면 뭉갠다 | 2026-07-16 |
| ❓ 물음표 | 획이 얇아 축소하면 존재감이 사라진다 | 2026-09-05 |
| 🙋 손 든 사람 | 글리프가 상반신에서 평평하게 끊겨 잘린 것처럼 보인다 | 2026-09-05 |
| 🧮 주판 | 갈색 프레임이라 앰버·오렌지 칩과 색이 붙는다. 다른 칩 색에서는 쓸 수 있으나 디테일이 잘다 | 2026-09-06 |

**고르는 기준 세 가지.** 이 셋을 통과하면 대체로 한 번에 끝난다.
1. **윤곽이 닫힌 단일 도형인가** — 얼굴(😴)이나 단일 사물(💰)은 통과, 반신 인물이나 여러 요소가 겹친 그림은 탈락
2. **글리프 안에 영문·숫자 텍스트가 없는가** — 종이·문서 계열 이모지가 특히 위험하다. 의심되면 1080px로 캡처해 Read로 열어 확인한다
3. **칩 색과 이모지 주색이 대비되는가** — 금색 💰은 앰버 칩에 붙고, 초록 ✅은 에메랄드 칩에 붙는다. 이모지 색을 먼저 정하고 칩 색을 고른다

**검수 절차(필수).** 1080px 캡처를 Read로 확인한 뒤, **112px로 축소한 임시본을 따로 캡처해 한 번 더 눈으로 본다.**
`sips -Z 112 <원본> --out <임시본>`이면 된다. 확인 후 임시본은 지운다. 1080px에서 멀쩡해 보여도 축소하면 무너지는 경우가 실제로 반복됐다.
...
```html
<div class="canvas" style="background: #F3F4F6; padding: 80px;">
  <!-- 타이틀 -->
...
- 배경 밝음(캔버스): `#F3F4F6` (gray-100) — **순백 `#FFFFFF`를 캔버스 배경으로 쓰지 않는다.** gray-50(`#F9FAFB`)은 흰색과 밝기 차이가 2%뿐이라 화면에 따라 경계가 안 보인다. 아티클 본문 배경이 흰색이라 이미지 경계가 사라지고, 이미지 안쪽 여백이 글 여백에 겹쳐 표가 안으로 밀려 보인다
- 밝은 캔버스 위의 카드·콜아웃: `#FFFFFF` — 캔버스가 gray-50이므로 카드를 흰색으로 두면 자연스럽게 떠 보인다. 반대로 하지 않는다
- 배경 밝음(보조): `#EEF4FF`
- 보조 텍스트·캡션: `#475569` (slate-600) 이상으로 어둡게 쓴다. **`#94A3B8`(slate-400)는 밝은 캔버스에서 명암비 2.3:1로 WCAG AA(4.5:1) 미달이다.** 의도적으로 약하게 보여야 하는 텍스트도 `#64748B`(slate-500)까지만 내린다
