...
Agent(
  ...
  작업:
  - **먼저 허브 중복을 확인한다.** docs/content/발행-완료.md에서 같은 클러스터의 허브·인접 글을
    찾아 해당 .mdx를 전수로 읽고, 브리프에 '중복 회피 목록'을 남긴다. 계획서 각도의 절반 이상이
    이미 다뤄졌으면 각도 재정의가 필요하다고 보고하고 대안을 2개 이상 낸다
  ...
  members: [
    {
      ...
      '중복 회피 목록'에서 링크 위임으로 표시된 항목은 본문에서 다시 쓰지 않는다.
...
Agent(
  ...
  - 브리프의 '중복 회피 목록'과 대조해 허브 글 내용이 재서술됐는지 확인한다 (재서술은 🔄 수정 필요)
...
grep -c '\*\*' apps/web/.velite/blog.json   # 0이어야 한다 (0일 때 grep 종료 코드가 1인 것은 정상)
```

- ⚠️ 체크리스트(`~` 이스케이프, 볼드 닫힘 구두점 등)는 사람/에이전트의 자가 점검이라 놓칠 수 있다 — `velite build`가 실제 게이트다
- `grep -c '\*\*'` 결과는 **`0`이어야 한다.** 볼드 미적용은 빌드 에러를 내지 않고 조용히 통과해 화면에 `**`가 그대로 노출되므로, 에러 0건만으로는 잡히지 않는다. 0이 아니면 해당 위치를 찾아 구두점을 굵게 밖으로 빼거나 `<strong>`으로 바꾼 뒤 재빌드한다
